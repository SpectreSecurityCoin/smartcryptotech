...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("smartcryptotech"))
        return false;
...
{
    // Convert smartcryptotech:// to smartcryptotech:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("smartcryptotech://"))
    {
        uri.replace(0, 12, "smartcryptotech:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SmartCryptoTech.lnk";
}
...
{
    return GetAutostartDir() / "smartcryptotech.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=SmartCryptoTech\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SmartCryptoTech-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  smartcryptotech-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SmartCryptoTech-Qt"));
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(24,24,24); }" // dark gray
                      ...
                      "QMenu::item:selected { background-color: rgb(48,140,198); }" // blue
                      ...
                      "QTreeView::item:selected { background-color: rgb(48,140,198); }" // blue
